...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{Capstanr2}
	\caption{Free body diagram of differential capstan problem. \citep{capstanman}}
	\label{fig:Capstan}
\end{figure}

Based on this, what is of question is how to extrapolate a pressure $p$ from the applied tension $T$. This can be done performing a force balance in the normal direction $\hat{n}$ which reduces to Equation~\ref{eq:CapstanSigRad}.
...
By assuming a small change in angle, the substitution of $\sin \theta \approx \theta$ and $dT d\theta/2 \approx 0$ are be made. Applying these relations to \ref{eq:CapstanSigRad} leaves \ref{eq:diffNormal}.
...
With the Python Script from Appendix A the convergence plot from Figure~\ref{fig:2_vii1_cnvg} below was created.
\begin{figure}[H]
    \centering
...
As per subsection 4.4.5 of \citep{ASMEbvpcVII2}, the following procedure was used to calculate the required thickness. Note that only the main formulas will be presented. For intermediate steps, see  the Python script in Appendix A. The following list of steps were carried out as per \citep{ASMEbvpcVII2}.
...
The Python Script in Appendix A output the following convergence plot as seen in Figure~\ref{fig:2_vii2_cnvg}.
\begin{figure}[H]
    \centering
...
As per subsection 8.5.2.2 of \citep{EN134453}, the following procedure was used to calculate the required thickness $e_a$. Note that the script in Appendix~A utilize SI units as per \citep{EN134453}.
...
Again, with the respective Python Script in Appendix A, a convergence plot was created (see Figure~\ref{fig:2_en13445_cnvg}).
\begin{figure}[H]
    \centering
